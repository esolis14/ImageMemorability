...
fileval = load_split('/Users/david/powerai-image-memorability/lamem/splits/val_1.txt')[:2000]
...
model.compile(
    "adam", "mse", metrics=[tf.keras.metrics.MeanAbsoluteError()])

# plot_model(model, to_file='model_plot.png', show_shapes=True, show_layer_names=True)

history= model.fit(x_train, y_train, batch_size=64*2, epochs=100, verbose=1, validation_data=(x_val, y_val), workers=8, use_multiprocessing=True)
...
# Plot accuracy
plt.plot(epochs, loss, 'b', label='Training MSE loss')
plt.plot(epochs, val_loss, 'r', label='Validation MSE loss')
plt.title('Training and validation MSE loss')
plt.legend()
plt.show()

# Plot loss
plt.plot(epochs, mean_absolute_error, 'b', label='Training MAE loss')
plt.plot(epochs, val_mean_absolute_error, 'r', label='Validation MAE loss')
plt.title('Training and validation MAE loss')
plt.legend()
plt.show()

# print(test_images_x[0])
# print(x_test[0])
